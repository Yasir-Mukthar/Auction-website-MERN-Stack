...
import { deleteSingleAuctionById, getSellerAuction } from "../store/auction/auctionSlice";
    ...
    <div className='bg-green-800 text-white'>
     ...
     
     {
      sellerAuction?.auctions?.map(auction =>(
        <div key={auction?._id} className='flex justify-between items-center p-4 border-b border-green-600'>
          <img src={auction?.image} alt="auction image" className='w-[50px] h-[50px] rounded-full' />
          ...
          <button onClick={()=>handleDeleteAuction(auction?._id)}>delete</button>
